...
parser = argparse.ArgumentParser(
	description='Tool to retrieve GroupMe messages and output them to a CSV file.')
parser.add_argument(
	'token', help='Access token used to authenticate yourself when making API requests.')
parser.add_argument(
	'-a', '--all', help='Retrieve all groups', action="store_true")
parser.add_argument(
	'-d', '--dm', help='Retrieve all direct messages', action="store_true")
parser.add_argument(
	'-g', '--group', help='Name of group to retrieve. Run without this flag to see list of groups.')
parser.add_argument(
	'-c', '--csv', help='Name of csv file to write to.', default='temp.csv')
parser.add_argument('-o', '--overwrite',
					help='overwrite csv file', action="store_true")
...
def getGroup(group_id, direct_msgs=False):
	if direct_msgs:
		...
		group = get(requests.get(
			URL + '/direct_messages' + TOKEN, params=params))
	else:
	...
	while (curCount < totalCount):
		...
		for msg in msgs:
			...
			if user not in users:
				users[user] = []
			if csv_file:
				wr.writerow([group_name, created_at.encode('utf-8'),
							 user.encode('utf-8'), text.encode('utf-8'), likes])
...
def main(retrieve_all, direct_msgs, group_id, csv_file, overwrite):
	if direct_msgs:
		groups = getDMs()
	else:
		groups = getGroups()
	...
	if retrieve_all:
		for k, v in groups.iteritems():
			...
			count, _ = countMsgs(
				k, v['id'], direct_msgs, csv_file=new_csv_file)
		...
		# print(group_name)
		# group_name = list(groups.keys())[list(groups.values()).index(group_id)] # Prints george
		if csv_file and os.path.isfile(csv_file) and not overwrite:
			raise IOError("File already exists. Try setting --overwrite.")
		if not csv_file:
			csv_file = 'messages.csv'
		count, _ = countMsgs(
			"suhdude", group_id, direct_msgs, csv_file=csv_file)
		print ("Processed {} messages. Wrote to {}.".format(count, csv_file))
	else:
...
if __name__ == "__main__":
	...
	if len(token) not in [32, 40]:
		raise IOError(
			"Invalid token. Please enter a 32-char or 40-char string.")
	TOKEN = "?token=" + token
